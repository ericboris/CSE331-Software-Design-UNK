...
public final class RatPoly {
    ...
    public RatTerm getTerm(int deg) {
        // {Inv: term rt at index_rt s.t. index_rt for all 0 <= index_rt < terms length}
    ...
    /**
     * Returns the terms of the given RatPoly as a list
     *
     * @param p - a RatPoly to get the terms of
     * @return - the terms of the given RatPoly
     */
    ...
    private static void scaleCoeff(List<RatTerm> lst, RatNum scalar) {
        // {Inv: rt_i.coeff = rt_i.coeff * scalar for all rt_i s.t. 0 <= rt_i < lst size}
    ...
    private static void incremExpt(List<RatTerm> lst, int degree) {
        // {Inv: rt_i.expt = rt_i.expt + degree for all rt_i s.t. 0 <= rt_i < lst size}
        for (int i = 0; i < lst.size(); i++) {
    ...
    private static void sortedInsert(List<RatTerm> lst, RatTerm newTerm) {
        if (!newTerm.getCoeff().equals(RatNum.ZERO)) { // otherwise, do nothing if the newTerm equals zero
            int lstSize = lst.size();
            // {Inv: for all 0 < i < lst size either insert newTerm or continue}
            for (int i = 0; i < lstSize; i++) {
                RatTerm rt = lst.get(i);
                // if the newTerm has the same exponent as the current index
                if (newTerm.getExpt() == rt.getExpt()) {
                    // then add the two
                    lst.set(i, rt.add(newTerm));
                    break;
                // if the new term has an exponent larger than that at the current index
                } else if (newTerm.getExpt() > rt.getExpt()) {
                    // shift everything from the current index on to the right and add new term at the current index
                    lst.add(i, newTerm);
                    break;
                // if the new term has a smaller exponent than everything encountered so far in the list
            ...
            // if none of the comparisons worked, because the list was empty
            if (lst.size() == 0) {
                // just add the new term to the empty list
                lst.add(newTerm);
            }
        }
    }
    ...
    public RatPoly negate() {
        ...
        List<RatTerm> negTerms = list(this);
        // {Inv: for all 0 <= i < negTerms size negTerm_i = negTerm_i * -1}
        for (int i = 0; i < negTerms.size(); i++) {
            negTerms.set(i, negTerms.get(i).negate());
        }

        return new RatPoly(negTerms);
    }
    ...
    public RatPoly add(RatPoly p) {
        ...
        List<RatTerm> r = list(this);
        // {Inv: r = q + p0 + p1 + ... + pi-1, where pj is the jth term in p}
        for (RatTerm tp : p.terms) {
            RatTerm tr = getTerm(tp.getExpt());
            int i = r.indexOf(tr);
            if (!tr.isZero() && tr.getExpt() == tp.getExpt()) {
                RatTerm result = tr.add(tp);
                if (result.isZero()) {
                    r.remove(i);
                } else {
                    r.set(i, result);
    ...
    public RatPoly mul(RatPoly p) {
        ...
        // {Inv: p = m * n_0 * n_1 * ... * n_{i-1} where n_i is the ith term in n}
        for (RatTerm pt : p.terms) {
            List<RatTerm> r = list(this);

            scaleCoeff(r, pt.getCoeff());
            incremExpt(r, pt.getExpt());

            result = result.add(new RatPoly(r));
        }
    ...
    public RatPoly div(RatPoly p) {
        ...
        List<RatTerm> remainder = list(this);
        List<RatTerm> quotient = new ArrayList<RatTerm>();

        RatTerm maxDivisor = p.terms.get(0);
        RatTerm maxDividend = remainder.get(0);

        // {Inv: r = r - r_0 - r_1 ... - r_{i-1} where r_i is the ith term in r}
        while (maxDivisor.getExpt() <= maxDividend.getExpt() && !maxDividend.isZero()) {
            RatTerm tq = maxDividend.div(maxDivisor);
            quotient.add(tq);

            List<RatTerm> tempProd = list(p);
            scaleCoeff(tempProd, tq.getCoeff());
            incremExpt(tempProd, tq.getExpt());

            RatPoly tProd = new RatPoly(tempProd);
            remainder = list(new RatPoly(remainder).sub(tProd));

            if (remainder.size() != 0) {
                maxDividend = remainder.get(0);
            } else {
                maxDividend = RatTerm.ZERO;
            }
        }
    ...
    public RatPoly differentiate() {
        ...
        List<RatTerm> termsList = list(this);
        // {Inv: for all 0 <= i < termsList size term_i = d(term_i) / dx}
        for (int i = 0; i < termsList.size(); i++) {
            RatTerm result = termsList.get(i).differentiate();
            if (!result.isZero()) {
                termsList.set(i, result);
            } else {
                termsList.remove(i);
            }
        }

        return new RatPoly(termsList);
    }
    ...
    public RatPoly antiDifferentiate(RatNum integrationConstant) {
        ...
        // {Inv: for all 0 <= i < termsList size term_i = antiderivative(term_i)}
        for (int i = 0; i < termsList.size(); i++) {
            termsList.set(i, termsList.get(i).antiDifferentiate());
        }
    ...
    public double integrate(double lowerBound, double upperBound) {
        ...
        RatPoly antiUpperBound = antiDifferentiate(RatNum.ZERO);
        double upperBoundVal = antiUpperBound.eval(upperBound);

        RatPoly antiLowerBound = antiDifferentiate(RatNum.ZERO);
        double lowerBoundVal = antiLowerBound.eval(lowerBound);

        return upperBoundVal - lowerBoundVal;
    }
    ...
    public double eval(double d) {
        ...
        List<RatTerm> termsList = list(this);
        double sum = 0.0;
        // {Inv: sum = 0 + t_0 + t_1 + t_2 + ... + t_i-1 where t_i is the ith term in termsList}
        for (int i = 0; i < termsList.size(); i++) {
            sum += termsList.get(i).eval(d);
        }
        return sum;
    }
