...
public final class RatPolyStack implements Iterable<RatPoly> {
    ...
    public RatPoly getNthFromTop(int index) {
        Stack<RatPoly> s = new Stack<RatPoly>();
        // {Inv: s = push(t_0), push(t_1-1), ..., push(t_i) and
        // polys = pop(t_0), pop(t_1-1), ..., pop(t_i)
        // where t_i is the ith term in polys}
        for (int i = 0; i < index; i++) {
            s.push(pop());
        }
        RatPoly nthFromTop = pop();
        push(nthFromTop);
        // {Inv: polys = push(t_0), push(t_1-1), ..., push(t_i) and
        // s = pop(t_0), pop(t_1-1), ..., pop(t_i)
        // where t_i is the ith term in s}
        while (!s.empty()) {
            push(s.pop());
        }
        return nthFromTop;
