...
public final class RatTerm {
    ...
    public RatTerm(RatNum c, int e) {
        this.coeff = c;
        this.expt = c.equals(RatNum.ZERO) ? 0 : e;
        checkRep();
    }
    ...
    public RatTerm add(RatTerm arg) {
        if (this.expt != arg.expt && !(this.isNaN() || this.isZero() || arg.isNaN() || arg.isZero())) {
            throw new IllegalArgumentException("Exponents must match.");
        }
        ...
        RatNum rn = coeff.add(arg.getCoeff());
        return new RatTerm(rn, expt);
    }
    ...
    public RatTerm sub(RatTerm arg) {
        ...
        RatNum rn = coeff.sub(arg.getCoeff());
        return new RatTerm(rn, expt);
    }
    ...
    public RatTerm div(RatTerm arg) {
        ...
        return new RatTerm(coeff.div(arg.getCoeff()), expt - arg.getExpt());
    }
